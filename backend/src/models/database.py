...
from shared.schemas.completion_event import CompletionMethodEnum
...
class GuideSessionModel(Base):
    ...
    guide_id = Column(UUID(as_uuid=True), ForeignKey("step_guides.guide_id"), nullable=False)
    current_step_identifier = Column(String(10), nullable=False, default="0")  # Support sub-indices like "1a", "1b"
    status = Column(String, nullable=False, default=SessionStatus.ACTIVE.value, index=True)
    completion_method = Column(String, nullable=False, default=CompletionMethod.HYBRID.value)
    ...
    completion_events = relationship("CompletionEventModel", back_populates="session", cascade="all, delete-orphan")
    progress_tracker = relationship("ProgressTrackerModel", back_populates="session", uselist=False, cascade="all, delete-orphan")
...
class StepGuideModel(Base):
    ...
    guide_data = Column(JSON, nullable=False)  # Stores full structured guide with sections
    adaptation_history = Column(JSON, nullable=True, default=list)  # Track all adaptations made
    last_adapted_at = Column(DateTime(timezone=True), nullable=True)  # Last adaptation timestamp
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    # Relationships
    sections = relationship("SectionModel", back_populates="guide", cascade="all, delete-orphan", order_by="SectionModel.section_order")
    steps = relationship("StepModel", back_populates="guide", cascade="all, delete-orphan", order_by="StepModel.step_index")
    sessions = relationship("GuideSessionModel", back_populates="guide")
    llm_requests = relationship("LLMGenerationRequestModel", back_populates="generated_guide")
...
class SectionModel(Base):
    ...
    guide_id = Column(UUID(as_uuid=True), ForeignKey("step_guides.guide_id"), nullable=False)
    section_identifier = Column(String(100), nullable=False)  # e.g., "setup", "configuration"
    ...
    steps = relationship("StepModel", back_populates="section", cascade="all, delete-orphan", order_by="StepModel.step_index")

    # Constraints
    __table_args__ = (
        UniqueConstraint("guide_id", "section_order", name="unique_section_order_per_guide"),
        UniqueConstraint("guide_id", "section_identifier", name="unique_section_identifier_per_guide"),
        CheckConstraint("section_order >= 0", name="positive_section_order"),
        CheckConstraint("estimated_duration_minutes > 0", name="positive_section_duration"),
    )
...
class StepModel(Base):
    ...
    step_identifier = Column(String(10), nullable=False, default="0")  # Support sub-indices like "1a", "1b"
    step_status = Column(String, nullable=False, default=StepStatus.ACTIVE.value)  # For adaptation
    ...
    prerequisites = Column(ARRAY(String), nullable=False, default=[])  # String descriptions of prerequisites
    ...
    __table_args__ = (
        ...
        CheckConstraint("estimated_duration_minutes > 0", name="positive_step_duration"),
    )
...
class CompletionEventModel(Base):
    ...
    session_id = Column(UUID(as_uuid=True), ForeignKey("guide_sessions.session_id"), nullable=False)
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("validation_score IS NULL OR (validation_score >= 0.0 AND validation_score <= 1.0)", name="valid_validation_score"),
...
class ProgressTrackerModel(Base):
    ...
    session_id = Column(UUID(as_uuid=True), ForeignKey("guide_sessions.session_id"), nullable=False, unique=True)
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("completion_percentage >= 0.0 AND completion_percentage <= 100.0", name="valid_completion_percentage"),
        CheckConstraint("estimated_time_remaining_minutes >= 0", name="positive_estimated_time"),
...
class LLMGenerationRequestModel(Base):
    ...
    generated_guide_id = Column(UUID(as_uuid=True), ForeignKey("step_guides.guide_id"), nullable=True)
...
class UserSessionModel(Base):
    ...
    active_guide_sessions = Column(ARRAY(UUID(as_uuid=True)), nullable=False, default=[])
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
