...
from shared.schemas.api_responses import (
    GuideGenerationRequest, GuideGenerationResponse, GuideDetailResponse
)
...
@router.post("/generate", response_model=GuideGenerationResponse, response_model_exclude_none=True)
...
@router.get("/{guide_id}", response_model=GuideDetailResponse, response_model_exclude_none=True)
async def get_guide(
    guide_id: uuid.UUID,
    current_user: str = Depends(get_current_user),
    llm_service = Depends(get_llm_service),
...
@router.get("/", response_model=list[GuideDetailResponse], response_model_exclude_none=True)
