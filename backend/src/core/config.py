...
class Settings(BaseSettings):
    ...
    lm_studio_model: str = Field(
        default="local-model", description="LM Studio model identifier"
    )
    enable_lm_studio: bool = Field(
        default=True, description="Enable LM Studio local LLM support"
    )

    # Rate limiting
    rate_limit_requests_per_minute: int = Field(
        default=60, description="Rate limit for API requests per minute"
    )
    guide_generation_rate_limit: int = Field(
        default=10, description="Rate limit for guide generation per minute"
    )

    # Performance
    max_guide_steps: int = Field(
        default=20, description="Maximum number of steps in a guide"
    )
    guide_generation_timeout_seconds: int = Field(
        default=30, description="Timeout for guide generation"
    )

    # Features
    enable_desktop_monitoring: bool = Field(
        default=False, description="Enable desktop monitoring features"
    )
    enable_websockets: bool = Field(
        default=True, description="Enable WebSocket connections"
    )
