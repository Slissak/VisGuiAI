...
from shared.schemas.guide_session import (
    GuideSession, GuideSessionCreate, GuideSessionUpdate,
    SessionStatus, CompletionMethod
)
...
from ..models.database import (
    GuideSessionModel, StepGuideModel, ProgressTrackerModel, StepModel
)
from ..core.redis import SessionStore
from ..exceptions import SessionNotFoundError, GuideNotFoundError, ValidationError
...
class SessionService:
    ...
    def __init__(self, guide_service: GuideService = None, session_store: SessionStore = None):
    ...
    async def create_session_simple(
        ...
        guide_query = select(StepGuideModel).options(
    ...
    async def update_session(
        ...
        if not self._is_valid_status_transition(session_model.status, request.status):
            raise InvalidSessionStateError(
                from_status=str(session_model.status),
    ...
    async def advance_to_next_step(
        ...
        if not next_identifier:
            ...
            await self.update_session(
                session_id,
                SessionUpdateRequest(status=SessionStatus.COMPLETED),
                db
            )
            return False
        ...
        update_query = update(GuideSessionModel).where(
            ...
            current_step_identifier=next_identifier,
    ...
    async def _cache_session_data(self, session: GuideSessionModel, guide, db: AsyncSession):
        """Cache session data in Redis."""
        session_data = {
    ...
    def _convert_progress_tracker(self, progress_model: ProgressTrackerModel) -> ProgressTracker:
    ...
    def _build_session_detail_from_cache(self, cached_data: dict) -> SessionDetailResponse:
    ...
    def _find_step_by_identifier(self, steps: List, identifier: str) -> Optional:
        ...
        for step in steps:
            ...
            if (hasattr(step, 'step_identifier') and step.step_identifier == identifier) or \
               (hasattr(step, 'step_index') and str(step.step_index) == identifier):
                return step
        return None
    ...
    def _get_next_step_identifier(self, steps: List, current_identifier: str) -> Optional[str]:
        ...
        for i, step in enumerate(steps):
            if (hasattr(step, 'step_identifier') and step.step_identifier == current_identifier) or \
               (hasattr(step, 'step_index') and str(step.step_index) == current_identifier):
                current_index = i
                break
