...
from shared.schemas.progress_tracker import ProgressTracker, ProgressUpdate
from shared.schemas.api_responses import ProgressResponse

from ..models.database import ProgressTrackerModel, GuideSessionModel, StepModel
...
class ProgressService:
    ...
    async def update_progress(
        ...
        if not progress_model:
            raise ProgressNotFoundError(f"Progress tracker for session {session_id} not found")
        ...
        if update.estimated_time_remaining_minutes is not None:
            update_data["estimated_time_remaining_minutes"] = update.estimated_time_remaining_minutes
    ...
    async def calculate_time_estimates(
        ...
        remaining_step_ids = [uuid.UUID(step_id) for step_id in progress_model.remaining_steps]
        ...
        if remaining_step_ids:
            ...
            base_remaining_time = sum(step.estimated_duration_minutes for step in remaining_steps)
        else:
        ...
        return {
            ...
            "average_time_per_completed_step": (
                progress_model.time_spent_minutes / completed_count if completed_count > 0 else 0
            )
        }
        ...
        analytics = {
            ...
            "efficiency_metrics": {
                "steps_per_hour": (
                    ...
                    if progress.time_spent_minutes > 0 else 0
                ),
                "estimated_completion_time": (
                    progress.started_at + timedelta(
                        minutes=progress.time_spent_minutes + time_estimates["estimated_time_remaining_minutes"]
                    )
                ).isoformat()
            },
            "status": {
                "current_step_id": str(progress.current_step_id) if progress.current_step_id else None,
    ...
    async def _get_cached_progress(self, session_id: uuid.UUID) -> Optional[ProgressResponse]:
...
async def get_progress_service(session_store: SessionStore = Depends(get_session_store)) -> ProgressService:
